...
```
  ...
  <verify>
Run `npx tsc --noEmit` — zero errors. Verify `grep "bullmq" package.json` shows dependency. Verify `grep "ioredis" package.json` shows dependency. Verify docker-compose.yml contains redis service. Verify src/config/redis.ts exists. Verify src/db/migrations/003_reminders.sql exists with UNIQUE constraint. Verify src/reminders/types.ts exists.
  </verify>
  <done>
BullMQ and ioredis installed. Redis added to docker-compose. Redis connection factory created with BullMQ-compatible settings. Database migration for sent_reminders table with composite unique constraint on (event_id, event_start). Reminder type definitions created. TypeScript compiles cleanly.
  </done>
</task>
...
- Determine recipients: get all phone numbers from `deps.familyWhitelist` (need to add `getAllPhoneNumbers(): string[]` method or iterate). For now, send to all whitelisted members individually using `sendSignalMessage`.
...
- CRITICAL: The worker needs a SEPARATE Redis connection from the queue (BullMQ requirement — queue and worker must not share the same ioredis instance). Accept two connections or create one internally.
  </action>
  <verify>
