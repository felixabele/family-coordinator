...
must_haves:
  ...
  artifacts:
    - path: "src/calendar/recurring.ts"
      provides: "RRULE formatting and next occurrence calculation"
      exports: ["formatRRule", "calculateNextOccurrences"]
...
```
  ...
  <verify>
Run `npx tsc --noEmit` — zero errors. Verify src/calendar/recurring.ts exists with formatRRule and calculateNextOccurrences exports. Verify src/llm/types.ts contains RecurrenceEntities. Verify src/llm/intent.ts tool schema includes recurrence property.
  </verify>
  <done>
RecurrenceEntities type and Zod schema added to LLM types. Tool schema extended with recurrence object. RRULE utility created with formatRRule (UTC UNTIL handling) and calculateNextOccurrences. CreateRecurringEventInput type defined. TypeScript compiles cleanly.
  </done>
</task>
...
5. `grep -r "createRecurringEvent" src/calendar/operations.ts` returns the function
6. `grep -r "Wiederkehrende Termine" src/llm/prompts.ts` returns the prompt section
7. `grep -r "nachmittags.*15:00" src/llm/prompts.ts` confirms updated time default
8. `grep -r "abends.*19:00" src/llm/prompts.ts` confirms updated time default
9. `grep -r "RRULE" src/calendar/recurring.ts` returns RRULE formatting code
10. `grep -r "recurrence" src/llm/intent.ts` returns tool schema extension
</verification>
