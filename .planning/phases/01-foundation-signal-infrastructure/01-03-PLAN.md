...
<task type="auto">
  <name>Task 1: Migrate idempotency to PostgreSQL and delete obsolete WhatsApp code</name>
  <files>
    src/state/idempotency.ts
    src/db/migrations/002_idempotency.sql
  </files>
  <action>
    ...
    **2. Delete obsolete WhatsApp code:**
  ...
  <verify>
    ...
    Verify idempotency uses pg: `grep "Pool" src/state/idempotency.ts` should match.
    Verify no ioredis import: `grep "ioredis" src/state/idempotency.ts` should return nothing.
  </verify>
  <done>
    ...
    No references to ioredis in idempotency module.
  </done>
</task>

<task type="auto">
  <name>Task 2: Rewrite entry point as Signal daemon and create .env.example</name>
  <files>
    src/index.ts
    .env.example
  </files>
  <action>
    **1. Rewrite src/index.ts as Signal daemon entry point:**

    First, read `src/llm/client.ts` to confirm `createAnthropicClient` is exported and its exact signature. It exists in the current codebase (exports `createAnthropicClient(apiKey: string): Anthropic`).
    ...
    **Imports needed:**
    ...
    - `createAnthropicClient` from `./llm/client.js` (confirmed export -- read file to verify path)
  ...
  <verify>
    ...
    Verify createAnthropicClient import: `grep "createAnthropicClient" src/index.ts`
...
<task type="checkpoint:human-verify" gate="blocking">
  <name>Task 3: Verify Signal bot starts and processes messages</name>
  <files>src/index.ts</files>
  <action>
    Human verification checkpoint. No code changes -- verify the complete Signal bot daemon works end-to-end.
    ...
    **Test the bot:**
    ...
    4. Send the same message twice quickly -- only one response should come back (idempotency working, duplicate prevention confirmed)
  ...
  <done>User has typed "approved" confirming the bot receives Signal messages, extracts intent via Claude, responds correctly, and duplicate messages sent in quick succession result in only one response. Or user has reported issues that will be addressed in a gap closure plan.</done>
</task>
...
- User confirms bot works via checkpoint, including duplicate message prevention
  </success_criteria>
